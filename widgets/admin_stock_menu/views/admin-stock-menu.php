...
<?php if ($user->role == User::ROLE_WORKER_SHOP) {?>
    <div class="box box-info color-palette-box">
        <div class="box-body">
            <div class="text-center">
                <a href="<?=Yii::$app->urlManager->createUrl(['/worker_shop/stock/view', 'id'=>$model->id])?>">
                    <img class="" src="/admin_files/img/storage.png" width="50%">
                </a>
            </div>
            <ul class="company-left-menu">
                <li><a href="<?=Yii::$app->urlManager->createUrl(['/worker_shop/stock/stacks', 'id'=>$model->id])?>" class="btn btn-primary width-full">Стелажи</a></li>
                <li><a href="<?=Yii::$app->urlManager->createUrl(['/worker_shop/stock/products', 'id'=>$model->id])?>" class="btn btn-primary width-full">Товары</a></li>
            </ul>
        </div>
    </div>
<?php }?>