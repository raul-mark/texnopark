                    ...
                    <div class="row">
                        ...
                        <div class="col-xs-6">
                            <div style="margin-left:50px">
                                ...
                                <?php if ($model->image) {?>
                                    <br/>
                                    <a href="<?=Yii::$app->urlManager->createUrl(['/worker/default/remove-photo', 'id'=>$model->image->id])?>" class="edit-user remove-object"><i class="fa fa-times"></i> Удалить фото</a>
                                <?php }?>
                            </div>
                        </div>
                    </div>
...
$script = <<<JS
    $(document).on('change', '.select-drop-stock', function() {
        ...
        if (id) {
            $.ajax({
                url: '/stock/coming/get-stacks',
                type: 'post',
                data: {'id':id},
                success: function(data) {
                    if (data && data['data']) {
                        block.append('<option value="">Выбрать стелаж</option>');
    ...
    $(document).on('change', '.select-drop-stack', function() {
        ...
        if (id) {
            $.ajax({
                url: '/stock/coming/get-shelvings',
                type: 'post',
                data: {'id':id},
                success: function(data) {
                    if (data && data['data']) {
                        block.append('<option value="">Выбрать полку</option>');
                        for (var i in data['data']) {
